...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
  :root {
    ...
    --text:#EAE6E0; --text2:#9A958E; --text3:#8A8479; --text4:#5C574F;
  ...
  .tab { display:flex; flex-direction:column; align-items:center; gap:3px; background:rgba(255,255,255,0.03); border:none; cursor:pointer; padding:6px 10px; color:var(--text3); font-family:var(--font); transition:color 0.2s, background 0.2s; border-radius:10px; position:relative; }
  .tab.on { color:var(--text); background:rgba(255,255,255,0.08); }
  ...
  .btn-icon { background:rgba(255,255,255,0.035); border:none; cursor:pointer; padding:6px; color:var(--text3); border-radius:8px; transition:all 0.18s; display:flex; align-items:center; }
...
const SyncBadge = ({ saving, pending = 0, syncError = false }) => (
  <div className="sync-badge">
    <div className="sync-dot" style={{
      background: syncError ? "var(--wine)" : saving ? "var(--amber)" : pending > 0 ? "var(--amber)" : "var(--sage)"
    }} />
    {syncError ? `${pending} não sincronizado${pending>1?"s":""}` : saving ? "Salvando…" : pending > 0 ? `${pending} pendente${pending>1?"s":""}` : "Sincronizado"}
  </div>
);
...
const TurnoAtivo = ({ turno, onAction, onPausa, onRetomar, onEncerrar, onEncerrarDia, clinicas, missoes = [] }) => {
  ...
  // Timer: missão dia intermediário → conta desde chegada na clínica ou atendimento
  // Turno local ou 1º dia de missão → conta desde saída de casa
  const timerBase = isMissao && diaAtual > 1
    ? (turno.chegadaClinica || turno.atendimentoInicio)
    : (turno.saidaCasa || turno.atendimentoInicio);
  const timerLabel = isMissao && diaAtual > 1 ? "Chegada na clínica" : "Saída";
  const dur = useDuration(timerBase);
      ...
      {diaSuspenso ? (
        ...
        <>
          ...
          <div style={{ fontSize:12, color:"var(--text3)", fontWeight:300, marginBottom:22 }}>
            {timerBase ? `${timerLabel} às ${fmtTime(timerBase)}` : ""}
          </div>
...
// ── Volta Sheet (viagem de volta após último dia de missão) ───
const VoltaSheet = ({ turno, voltaForm, setVoltaForm, onConfirmar, onCancelar, saving, clinicas }) => {
  const [humor,    setHumor]   = useState("bom");
  const [desgaste, setDesgaste] = useState(3); // missão costuma ser mais cansativa
  const [transito, setTransito] = useState("moderado");
  const [obs,      setObs]     = useState("");

  const clinica = getClinicaById(turno.clinicaId, clinicas);

  // Calcula tempo de viagem de volta automaticamente
  const tempoVolta = voltaForm.partidaDestino && voltaForm.chegadaCasa
    ? new Date(voltaForm.chegadaCasa) - new Date(voltaForm.partidaDestino)
    : null;
    ...
    <>
      ...
      <div className="sheet" style={{ maxHeight:"88vh", overflowY:"auto" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:18 }}>
          <div className="cdot" style={{ width:10, height:10, background:clinica.cor }} />
          <div className="sh-title" style={{ margin:0 }}>Viagem de volta</div>
          <div style={{ marginLeft:"auto", fontSize:10, color:"var(--amber)", background:"rgba(160,140,96,0.12)", borderRadius:100, padding:"3px 10px" }}>✈ Missão encerrada</div>
        </div>

        <div style={{ fontSize:11, color:"var(--text3)", fontWeight:300, marginBottom:16 }}>
          Registre o horário da sua viagem de volta para calcular o desgaste real da missão.
        </div>
        ...
        <div className="frow" style={{ marginBottom:12 }}>
          <div className="fg">
            ...
            <input className="fi" type="datetime-local"
              value={voltaForm.partidaDestino}
              onChange={e => setVoltaForm(f => ({ ...f, partidaDestino: e.target.value }))} />
          </div>
          <div className="fg">
            <label className="fl">
              Chegada em casa
              {tempoVolta && tempoVolta > 0 && (
                <span style={{ color:"var(--sage)", fontWeight:300, textTransform:"none", letterSpacing:0, marginLeft:4 }}>
                  · {fmtDur(tempoVolta)} de viagem
                </span>
              )}
            </label>
            <input className="fi" type="datetime-local"
              value={voltaForm.chegadaCasa}
              onChange={e => setVoltaForm(f => ({ ...f, chegadaCasa: e.target.value }))} />
          </div>
        </div>
        ...
        <div className="fg" style={{ marginBottom:16 }}>
          <label className="fl">Observações</label>
          <textarea className="fi" rows={2} placeholder="Notas sobre a missão…" value={obs} onChange={e=>setObs(e.target.value)} style={{ resize:"none", lineHeight:1.6 }} />
        </div>
        ...
        <button className="btn-s" onClick={onCancelar}>Pular viagem de volta</button>
      </div>
    </>
  );
};
...
const TelaHistorico = ({ historico, setHistorico, clinicas, missoes = [], setMissoes }) => {
  ...
  const [addAtendMissao, setAddAtendMissao] = useState(null); // missao object
  const [addForm, setAddForm] = useState({});
  const [savingAdd, setSavingAdd] = useState(false);

  // ── Adicionar turno local manual ──
  const [showAddLocal, setShowAddLocal] = useState(false);
  const [addLocalForm, setAddLocalForm] = useState({});
  const [savingAddLocal, setSavingAddLocal] = useState(false);
  ...
  const salvarAddLocal = async () => {
    if (!addLocalForm.atendimentoInicio && !addLocalForm.saidaCasa) return;
    setSavingAddLocal(true);
    const novoTurno = {
      id:                uid(),
      clinicaId:         addLocalForm.clinicaId,
      missaoId:          null,
      diaMissao:         1,
      saidaCasa:         parseDatetimeLocal(addLocalForm.saidaCasa),
      chegadaClinica:    parseDatetimeLocal(addLocalForm.chegadaClinica),
      atendimentoInicio: parseDatetimeLocal(addLocalForm.atendimentoInicio),
      atendimentoFim:    parseDatetimeLocal(addLocalForm.atendimentoFim),
      chegadaCasa:       parseDatetimeLocal(addLocalForm.chegadaCasa),
      humor:             addLocalForm.humor || null,
      desgaste:          Number(addLocalForm.desgaste) || null,
      transito:          addLocalForm.transito || null,
      observacoes:       addLocalForm.observacoes || null,
      status:            "concluido",
      pausas:            [],
      manual:            true,
    };
    await saveTurno(novoTurno);
    setHistorico(h => [novoTurno, ...h]);
    setSavingAddLocal(false);
    setShowAddLocal(false);
  };

  const alf = (field) => (e) => setAddLocalForm(f => ({ ...f, [field]: e.target.value }));

  const abrirAddAtend = (missao, turnos) => {
    const proximoDia = turnos.length > 0 ? Math.max(...turnos.map(t => t.diaMissao || 1)) + 1 : 1;
    setAddForm({
      clinicaId:         missao.clinicaId,
      diaMissao:         proximoDia,
      saidaCasa:         "",
      chegadaClinica:    "",
      atendimentoInicio: "",
      atendimentoFim:    "",
    ...
    setAddAtendMissao(missao);
  };

  const salvarAddAtend = async () => {
    if (!addAtendMissao) return;
    if (!addForm.atendimentoInicio && !addForm.saidaCasa) return;
    setSavingAdd(true);
    const novoTurno = {
      id:                uid(),
      clinicaId:         addForm.clinicaId,
      missaoId:          addAtendMissao.id,
      diaMissao:         Number(addForm.diaMissao) || 1,
      saidaCasa:         parseDatetimeLocal(addForm.saidaCasa),
      chegadaClinica:    parseDatetimeLocal(addForm.chegadaClinica),
      atendimentoInicio: parseDatetimeLocal(addForm.atendimentoInicio),
      atendimentoFim:    parseDatetimeLocal(addForm.atendimentoFim),
      chegadaCasa:       parseDatetimeLocal(addForm.chegadaCasa),
      humor:             addForm.humor || null,
      desgaste:          Number(addForm.desgaste) || null,
      transito:          addForm.transito || null,
      observacoes:       addForm.observacoes || null,
      status:            "concluido",
      pausas:            addForm.pausas || [],
      manual:            true,
    };
    await saveTurno(novoTurno);
    setHistorico(h => [...h, novoTurno]);
    // Expande o bloco da missão automaticamente
    setMissaoExpand(e => ({ ...e, [addAtendMissao.id]: true }));
    setSavingAdd(false);
    setAddAtendMissao(null);
  };

  const af = (field) => (e) => setAddForm(f => ({ ...f, [field]: e.target.value }));
    ...
    <div className="content">
      <div className="ph" style={{ display:"flex", alignItems:"flex-end", justifyContent:"space-between" }}>
        ...
        <div style={{ display:"flex", gap:10, alignItems:"center" }}>
          <button className="btn-icon" title="Adicionar turno retroativo" onClick={abrirAddLocal} style={{ color:"var(--sage)" }}>
            <Icon d={ICONS.plus} size={18} stroke={1.6} />
          </button>
      ...
      {/* ── Sheet: turno local manual ── */}
      {showAddLocal && (
        <>
          <div className="overlay" onClick={() => setShowAddLocal(false)} />
          <div className="sheet" style={{ maxHeight:"88vh", overflowY:"auto" }}>
            <div className="sh-handle" />
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:4 }}>
              <div className="sh-title" style={{ margin:0 }}>Adicionar turno</div>
              <span style={{ fontSize:10, color:"var(--sage)", background:"rgba(126,155,138,0.12)", padding:"3px 8px", borderRadius:20, fontWeight:500 }}>retroativo</span>
            </div>
            <div style={{ fontSize:12, color:"var(--text3)", fontWeight:300, marginBottom:18 }}>Preencha o que tiver. Dados aproximados são válidos.</div>

            <div style={{ fontSize:10, letterSpacing:"1.5px", textTransform:"uppercase", color:"var(--text3)", marginBottom:10, fontWeight:400 }}>Contexto</div>
            <div className="fg" style={{ marginBottom:16 }}>
              <label className="fl">Clínica</label>
              <select className="fi" value={addLocalForm.clinicaId} onChange={alf("clinicaId")}>
                {clinicas.map(c => <option key={c.id} value={c.id}>{c.nome} — {c.cidade}</option>)}
              </select>
            </div>

            <div style={{ fontSize:10, letterSpacing:"1.5px", textTransform:"uppercase", color:"var(--text3)", marginBottom:10, fontWeight:400 }}>Horários <span style={{ color:"var(--text4)", textTransform:"none", letterSpacing:0, fontSize:10 }}>(preencha o que tiver)</span></div>
            {[
              ...
              <div key={field} className="fg" style={{ marginBottom:8 }}>
                ...
                <input className="fi" type="datetime-local" value={addLocalForm[field] || ""} onChange={alf(field)} />
              </div>
            ))}
            ...
            <div className="fg" style={{ marginBottom:8 }}>
              <label className="fl">Como foi o dia?</label>
              <div className="chips">
                {HUMOR.map(h => <button key={h.v} className={`chip${addLocalForm.humor===h.v?" sel":""}`} onClick={()=>setAddLocalForm(f=>({...f,humor:h.v}))}>{h.l}</button>)}
              </div>
            </div>
            <div className="fg" style={{ marginBottom:8 }}>
              <label className="fl">Desgaste físico</label>
              <div className="chips">
                {DESGASTE.map(d => (
                  <button key={d.v} className={`chip${addLocalForm.desgaste==d.v?" sel":""}`}
                    onClick={()=>setAddLocalForm(f=>({...f,desgaste:d.v}))}
                    style={addLocalForm.desgaste==d.v?{borderColor:d.cor,color:d.cor,background:`${d.cor}18`}:{}}>
                    <span style={{ fontFamily:"monospace", fontSize:13 }}>{d.sym}</span>{d.l}
                  </button>
                ))}
              </div>
            ...
            <div className="fg" style={{ marginBottom:8 }}>
              <label className="fl">Trânsito percebido</label>
              <div className="chips">
                {TRANSITO.map(t => <button key={t.v} className={`chip${addLocalForm.transito===t.v?" sel":""}`} onClick={()=>setAddLocalForm(f=>({...f,transito:t.v}))}>{t.l}</button>)}
              </div>
            </div>
            <div className="fg" style={{ marginBottom:16 }}>
              <label className="fl">Observações</label>
              <textarea className="fi" rows={2} value={addLocalForm.observacoes||""} onChange={alf("observacoes")} placeholder="Notas sobre o turno…" style={{ resize:"none", lineHeight:1.6 }} />
            </div>

            <button className="btn-p" onClick={salvarAddLocal} disabled={savingAddLocal}>
              {savingAddLocal ? "Salvando…" : "Adicionar turno"}
            </button>
            <button className="btn-s" onClick={() => setShowAddLocal(false)}>Cancelar</button>
          </div>
        </>
      )}
      ...
      {/* ── Sheet: adicionar atendimento manual ── */}
      {addAtendMissao && (
        <>
          <div className="overlay" onClick={() => setAddAtendMissao(null)} />
          <div className="sheet" style={{ maxHeight:"88vh", overflowY:"auto" }}>
            <div className="sh-handle" />
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:4 }}>
              <div className="sh-title" style={{ margin:0 }}>Adicionar atendimento</div>
              <span style={{ fontSize:10, color:"var(--amber)", background:"rgba(160,140,96,0.12)", padding:"3px 8px", borderRadius:20, fontWeight:500 }}>manual</span>
            </div>
            <div style={{ fontSize:12, color:"var(--text3)", fontWeight:300, marginBottom:18 }}>{addAtendMissao.cidade}</div>

            {/* Clínica e dia */}
            <div style={{ fontSize:10, letterSpacing:"1.5px", textTransform:"uppercase", color:"var(--text3)", marginBottom:10, fontWeight:400 }}>Contexto</div>
            <div className="fg" style={{ marginBottom:8 }}>
              <label className="fl">Clínica</label>
              <select className="fi" value={addForm.clinicaId} onChange={af("clinicaId")}>
                {clinicas.map(c => <option key={c.id} value={c.id}>{c.nome} — {c.cidade}</option>)}
              </select>
            </div>
            <div className="fg" style={{ marginBottom:16 }}>
              <label className="fl">Dia da missão</label>
              <input className="fi" type="number" min={1} value={addForm.diaMissao} onChange={af("diaMissao")} />
            </div>
            ...
            {[
              ...
              <div key={field} className="fg" style={{ marginBottom:8 }}>
                <label className="fl">{label}</label>
                <input className="fi" type="datetime-local" value={addForm[field] || ""} onChange={af(field)} />
              </div>
            ))}
            ...
            <div className="fg" style={{ marginBottom:8 }}>
              <label className="fl">Como foi o dia?</label>
              <div className="chips">
                {HUMOR.map(h => <button key={h.v} className={`chip${addForm.humor===h.v?" sel":""}`} onClick={()=>setAddForm(f=>({...f,humor:h.v}))}>{h.l}</button>)}
              </div>
            </div>
            <div className="fg" style={{ marginBottom:8 }}>
              <label className="fl">Desgaste físico</label>
              <div className="chips">
                {DESGASTE.map(d => (
                  <button key={d.v} className={`chip${addForm.desgaste==d.v?" sel":""}`}
                    onClick={()=>setAddForm(f=>({...f,desgaste:d.v}))}
                    style={addForm.desgaste==d.v?{borderColor:d.cor,color:d.cor,background:`${d.cor}18`}:{}}>
                    <span style={{ fontFamily:"monospace", fontSize:13 }}>{d.sym}</span>{d.l}
                  </button>
                ))}
              </div>
            ...
            <div className="fg" style={{ marginBottom:8 }}>
              <label className="fl">Trânsito percebido</label>
              <div className="chips">
                {TRANSITO.map(t => <button key={t.v} className={`chip${addForm.transito===t.v?" sel":""}`} onClick={()=>setAddForm(f=>({...f,transito:t.v}))}>{t.l}</button>)}
              </div>
            </div>
            <div className="fg" style={{ marginBottom:16 }}>
              <label className="fl">Observações</label>
              <textarea className="fi" rows={2} value={addForm.observacoes||""} onChange={af("observacoes")} placeholder="Notas sobre o turno…" style={{ resize:"none", lineHeight:1.6 }} />
            </div>

            <button className="btn-p" onClick={salvarAddAtend} disabled={savingAdd}>
              {savingAdd ? "Salvando…" : "Adicionar atendimento"}
            </button>
            <button className="btn-s" onClick={() => setAddAtendMissao(null)}>Cancelar</button>
          </div>
        </>
      )}
          ...
          return (
            <div key={missao.id} className="ti fu missao-group" style={{ animationDelay:`${gi*0.03}s`, borderLeft:"3px solid var(--amber)", paddingLeft:2 }}>
              ...
              <div className="ti-top" style={{ cursor:"pointer" }} onClick={() => setMissaoExpand(e=>({...e, [missao.id]:!e[missao.id]}))}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  <button className="btn-icon" style={{ padding:4, color:"var(--amber)" }} title="Adicionar atendimento"
                    onClick={e => { e.stopPropagation(); abrirAddAtend(missao, turnos); }}>
                    <Icon d={ICONS.plus} size={15} stroke={1.8} />
                  </button>
                      ...
                      <div key={t.id} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"8px 0", borderBottom:ti<turnos.length-1?"1px solid var(--border2)":"none" }}>
                        <div>
                          <div style={{ fontSize:12, fontWeight:500, display:"flex", alignItems:"center", gap:6 }}>
                            Dia {t.diaMissao || ti+1}
                            {t.manual && <span style={{ fontSize:9, color:"var(--amber)", background:"rgba(160,140,96,0.12)", padding:"1px 6px", borderRadius:10, fontWeight:500 }}>manual</span>}
                          </div>
        ...
        return (
          <div key={t.id} className="ti fu" style={{ animationDelay:`${i*0.03}s` }}>
            ...
            <div className="ti-tags">
              ...
              {t.manual && <span className="tag" style={{ color:"var(--sage)", background:"rgba(126,155,138,0.12)", fontSize:10 }}>manual</span>}
...
export default function App() {
  ...
  const [showVoltaForm, setShowVoltaForm] = useState(false);
  const [voltaForm,    setVoltaForm]   = useState({ partidaDestino:"", chegadaCasa:"" });
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // 4. Carregar missões — Supabase + localStorage merged
      try {
        const msSupabase = await loadMissoes();
        const msLocal    = JSON.parse(localStorage.getItem('meridian_missoes_v1') || '[]');
        // Merge: Supabase tem prioridade, mas mantém missões locais não sincronizadas
        const merged = [...msSupabase];
        msLocal.forEach(ml => {
          if (!merged.find(ms => ms.id === ml.id)) merged.push(ml);
        });
        setMissoes(merged);
      } catch {
        try {
      ...
      // 5. Tentar reenviar pendentes da sessão anterior
      const falhas = await flushPending();
      setPendingCount(falhas > 0 ? falhas : getPendingCount());
  ...
  // Wrapper de saveTurno que mantém pendingCount atualizado
  const doSaveTurno = useCallback(async (t) => {
    const result = await saveTurno(t);
    setPendingCount(getPendingCount());
    setSyncError(getPendingCount() > 0);
    return result;
  }, []);
  ...
  const iniciarTurno = useCallback(async (dados) => {
    ...
    setSaving(true);
    await doSaveTurno(novoTurno);
    setSaving(false);
  }, [setTurnoComPersistencia, doSaveTurno]);
  ...
  const iniciarMissaoDia = useCallback(async (missao) => {
    ...
    await doSaveTurno(novoTurno);
    setSaving(false);
  }, [setTurnoComPersistencia, historico, doSaveTurno]);

  const registrarAcao = useCallback(async (acao) => {
    const updated = { ...turno, [acao]: now() };
    setTurnoComPersistencia(updated);
    await doSaveTurno(updated);

    if (acao === "chegadaCasa") {
      const missao = turno.missaoId ? missoes.find(m => m.id === turno.missaoId) : null;
      const isUltimoDia = !missao || missao.noites === 0 || (turno.diaMissao || 1) > missao.noites;
      if (missao && isUltimoDia) {
        setVoltaForm({
          partidaDestino: fmtDatetimeLocal(updated.atendimentoFim),
          chegadaCasa:    fmtDatetimeLocal(updated.chegadaCasa),
        });
        setTimeout(() => setShowVoltaForm(true), 700);
      } else {
        setTimeout(() => setShowEncerrar(true), 700);
      }
    }
  }, [turno, missoes, setTurnoComPersistencia, doSaveTurno]);

  const encerrarDia = useCallback(async () => {
    if (!turno) return;
    const final = { ...turno, status: "concluido" };
    const { data } = await doSaveTurno(final);
  ...
  const confirmarVolta = useCallback(async (extras, voltaDados) => {
    if (!turno) return;
    setSaving(true);
    ...
    setHistorico(h => [data || final, ...h]);
    setShowEncerrar(false);
    setShowVoltaForm(false);

    if (turno.missaoId) {
      const missaoAtual = missoes.find(m => m.id === turno.missaoId) ||
        JSON.parse(localStorage.getItem('meridian_missoes_v1') || '[]').find(m => m.id === turno.missaoId);
      if (missaoAtual) {
        const missaoFechada = {
          ...missaoAtual,
          partidaDestino: voltaDados.partidaDestino ? new Date(voltaDados.partidaDestino).getTime() : null,
          chegadaCasa:    voltaDados.chegadaCasa    ? new Date(voltaDados.chegadaCasa).getTime()    : final.chegadaCasa,
          status:         "concluida",
        };
        await saveMissao(missaoFechada);
        setMissoes(ms => ms.map(m => m.id === missaoFechada.id ? missaoFechada : m));
  ...
  const pausarAtendimento = useCallback(async (motivo = null) => {
    ...
    setTurnoComPersistencia(updated);
    await doSaveTurno(updated);
  }, [turno, setTurnoComPersistencia, doSaveTurno]);
  ...
  const setClinicasComPersistencia = (cs) => {
    setClinicas(cs);
    saveClinicas(cs);
  };
      ...
      <div className="app-wrap">
        ...
        {showVoltaForm && turno && (
          <VoltaSheet
            turno={turno}
            voltaForm={voltaForm}
            setVoltaForm={setVoltaForm}
            onConfirmar={(extras) => confirmarVolta(extras, voltaForm)}
            onCancelar={() => { setShowVoltaForm(false); setTimeout(() => setShowEncerrar(true), 200); }}
            saving={saving}
            clinicas={clinicas}
          />
        )}
