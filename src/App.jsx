...
import { loadTurnos, saveTurno, saveCalculo, loadCalculos, loadClinicas as loadClinicasDB, saveClinica, deleteClinica, seedClinicas } from "./lib/db";
...
export default function App() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // 2. Carregar clínicas do Supabase (fonte de verdade entre dispositivos)
      try {
        const clinicasDB = await loadClinicasDB();
        if (clinicasDB === null) {
          // Supabase vazio ou offline — usa localStorage
          ...
          saveClinicas(CLINICAS_DEFAULT); // cópia local
        } else {
          setClinicas(clinicasDB);
          saveClinicas(clinicasDB); // sincroniza local
        }
      } catch {
        setClinicas(loadClinicas());
      }
