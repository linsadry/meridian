...
export default function App() {
  ...
  const registrarAcao = useCallback(async (acao) => {
    ...
    if (acao === "chegadaCasa") {
      ...
      if (missao && isUltimoDia) {
        // Missão: abre formulário de viagem de volta
        setVoltaForm({
          partidaDestino: fmtDatetimeLocal(updated.atendimentoFim), // sugestão
          chegadaCasa:    fmtDatetimeLocal(updated.chegadaCasa),
        });
        setTimeout(() => setShowVoltaForm(true), 700);
      } else {
        // Turno local: abre sheet de avaliação normal
        setTimeout(() => setShowEncerrar(true), 700);
      }
    }
  }, [turno, missoes, setTurnoComPersistencia]);
  ...
  const encerrarDia = useCallback(async () => {
    ...
    // Missão continua aberta para o próximo dia
  }, [turno]);

  // Confirma a viagem de volta e encerra a missão
  const confirmarVolta = useCallback(async (extras, voltaDados) => {
    ...
    // Fecha a missão com dados da viagem de volta
    if (turno.missaoId) {
