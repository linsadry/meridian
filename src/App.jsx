...
import { loadTurnos, saveTurno, saveCalculo, loadCalculos } from "./lib/db";
...
const TurnoAtivo = ({ turno, onAction, onEncerrar, clinicas }) => {
  ...
  return (
    <div className={`shift-card ${getFaseClass(turno)} fu`}>
      ...
      <div style={{ fontSize:10, letterSpacing:"2px", textTransform:"uppercase", color:"var(--text3)", marginBottom:4 }}>{getFaseLbl(turno)}</div>
      <div className="timer">{fmtDur(dur)}</div>
      <div style={{ fontSize:12, color:"var(--text3)", fontWeight:300, marginBottom:22 }}>Saída às {fmtTime(turno.saidaCasa)}</div>

      <div className="agrid">
        {FASES.map((f,i) => {
          const done = !!turno[f.key], cur = i === faseIdx;
          return (
      ...
      {turno.chegadaCasa && <button className="enc-btn" onClick={onEncerrar}>Encerrar turno</button>}
    </div>
  );
};
...
const StartShift = ({ onIniciar, clinicas, saving }) => {
  const [step, setStep]   = useState("tipo");
  const [tipo, setTipo]   = useState(null);
  const [clinica, setClin] = useState(null);
  const [meiosSel, setMeios] = useState([]);
  const [missao, setMissao] = useState({ cidade:"", noites:1, diasConsecutivos:1, diasOciosos:0, tempoViagemEstimadoH:0, hospedagemProxima:false });

  const toggleMeio = (m) => setMeios(prev => prev.includes(m) ? prev.filter(x=>x!==m) : [...prev, m]);
  const confirmar = (c = clinica, m = null) => onIniciar({ clinicaId:c.id, saidaCasa:now(), missao:m });
      ...
      {step === "tipo" && (
        <div className="tipo-list fu">
          {[
            ...
            { t:"viagem", icon:"plane",    title:"Viagem profissional", sub:"Missão em outra cidade" },
          ].map(({ t, icon, title, sub }) => (
            <button key={t} className="tipo-card" onClick={() => { setTipo(t); setStep("clinica"); }}>
              <div className="tipo-icon"><Icon d={ICONS[icon]} size={20} stroke={1.3} /></div>
      ...
      {step === "clinica" && (
        <>
          ...
          <div className="sheet">
            ...
            <div className="sh-sub">{tipo==="local"?"Atendimento local":"Missão em outra cidade"}</div>
            <div className="clin-list">
              {clinicas.filter(c => tipo==="local" ? c.tipo==="local" : true).map(c => (
                <button key={c.id} className={`clin-opt${clinica?.id===c.id?" sel":""}`}
                  onClick={() => {
                    setClin(c);
                    if (c.tipo==="viagem"||tipo==="viagem") { setMissao(m=>({...m,cidade:c.cidade})); setStep("missao"); }
                    else confirmar(c, null);
                  }}>
      ...
      {step === "missao" && clinica && (
        <>
          ...
          <div className="sheet">
            ...
            <div className="sh-sub">{clinica.nome} · {missao.cidade}</div>

            <div className="fg">
              <label className="fl">Meio(s) de transporte</label>
              <div className="chips">
                {MEIOS_TODOS.map(m => (
                  <button key={m} className={`chip${meiosSel.includes(m)?" sel":""}`} onClick={() => toggleMeio(m)}>
                    <Icon d={ICONS[MEIOS_ICONS[m]]} size={12} stroke={1.4} />{m}
                  </button>
                ))}
              </div>
              {meiosSel.length > 1 && (
                <div style={{ fontSize:11, color:"var(--text3)", marginTop:6, fontWeight:300 }}>
                  Ex.: Carro + Avião para viagens com transbordo
                </div>
              )}
            </div>
            <div className="frow">
              <div className="fg">
                <label className="fl">Tempo de viagem (h)</label>
                <input className="fi" type="number" min="0" max="30"
                  value={missao.tempoViagemEstimadoH}
                  onChange={e => setMissao(d=>({...d, tempoViagemEstimadoH:+e.target.value}))} />
              </div>
              <div className="fg">
                <label className="fl">Noites fora</label>
                <input className="fi" type="number" min="0" max="30"
                  value={missao.noites}
            ...
            <div className="frow">
              <div className="fg">
                <label className="fl">Dias de atendimento</label>
                <input className="fi" type="number" min="1" max="14"
              ...
              <div className="fg">
                <label className="fl" style={{ display:"flex", alignItems:"center", gap:4 }}>
                  Dias ociosos
                  <span title="Ex.: chegou sábado, atendimento começa segunda" style={{ cursor:"help", color:"var(--amber)" }}>
                    <Icon d={ICONS.alert} size={11} stroke={1.5} color="var(--amber)" />
                  </span>
                </label>
                <input className="fi" type="number" min="0" max="10"
                  value={missao.diasOciosos}
                  onChange={e => setMissao(d=>({...d, diasOciosos:+e.target.value}))} />
              </div>
            </div>
            <div className="fg">
            ...
            {missao.diasOciosos > 0 && (
              <div className="idle-card" style={{ margin:"0 0 12px", padding:"12px 14px" }}>
                <div className="idle-icon"><Icon d={ICONS.bed} size={15} stroke={1.4} /></div>
                <div className="idle-text">
                  <strong>{missao.diasOciosos} dia{missao.diasOciosos>1?"s":""} ocioso{missao.diasOciosos>1?"s":""}</strong> contabilizados no custo total da missão — desgaste acumulado e tempo longe de casa.
                </div>
              </div>
            )}
            <button className="btn-p" onClick={() => confirmar(clinica, { ...missao, meios: meiosSel.length ? meiosSel : ["Outro"] })} disabled={saving}>
...
const EncerrarSheet = ({ turno, onConfirmar, onCancelar, saving, clinicas }) => {
  ...
  const tlItems = [
    { ts:turno.saidaCasa,        lbl:"Saída de casa",         dur:ida },
    { ts:turno.chegadaClinica,   lbl:"Chegada na clínica",    dur:turno.atendimentoInicio&&turno.chegadaClinica?turno.atendimentoInicio-turno.chegadaClinica:null },
    { ts:turno.atendimentoInicio,lbl:"Início do atendimento", dur:atend },
    { ts:turno.atendimentoFim,   lbl:"Fim do atendimento",    dur:volta },
    { ts:turno.chegadaCasa,      lbl:"Chegada em casa",       dur:null },
  ].filter(i => i.ts);
  ...
  return (
    <>
      ...
      <div className="sheet">
        ...
        <div className="tl">
          {tlItems.map((item,i) => (
            <div className="tl-row" key={i}>
              ...
              <div className="tl-sp">
                <div className="tl-dot f" />
                {i < tlItems.length - 1 && <div className="tl-line" />}
              </div>
              <div className="tl-c">
                <div className="tl-lb">{item.lbl}</div>
                {item.dur > 0 && <div className="tl-dur">{fmtDur(item.dur)}</div>}
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:6, marginBottom:16, background:"var(--bg2)", borderRadius:"var(--r-sm)", padding:10 }}>
          {[{l:"Ida",v:fmtDur(ida)},{l:"Atendimento",v:fmtDur(atend)},{l:"Total fora",v:fmtDur(total)}].map(({l,v}) => (
            <div key={l} style={{ textAlign:"center" }}>
              <div style={{ fontFamily:"var(--font-d)", fontSize:16 }}>{v}</div>
...
const TelaHome = ({ turno, historico, onIniciar, onAction, onEncerrar, saving, clinicas }) => {
  ...
  return (
    <div className="content">
      <div className="ph">
        ...
        <div className="ph-title">{turno?"Turno ativo":"Bom dia"}</div>
        {!turno && <div className="ph-sub">Selecione o tipo de jornada</div>}
      </div>

      {turno ? (
        <TurnoAtivo turno={turno} onAction={onAction} onEncerrar={onEncerrar} clinicas={clinicas} />
      ) : (
        <div className="home-grid">
          <div className="home-grid-full">
            <StartShift onIniciar={onIniciar} clinicas={clinicas} saving={saving} />
          </div>
          ...
          <div className="card cp fu fu2">
            <div className="c-ey">Este mês</div>
            <div className="c-val-md">{historico.filter(t=>{const d=new Date(t.saidaCasa),h=new Date();return d.getMonth()===h.getMonth()&&d.getFullYear()===h.getFullYear();}).length}</div>
            <div className="c-sub">turnos registrados</div>
          </div>
        </div>
      )}
    </div>
...
const TelaHistorico = ({ historico, setHistorico, clinicas }) => {
  const [confirmDelete, setConfirmDelete] = useState(null); // id do turno a deletar
  ...
  const handleDelete = (id) => {
    ...
    try {
      const all = JSON.parse(localStorage.getItem("meridian_turnos_v1")||"[]");
      localStorage.setItem("meridian_turnos_v1", JSON.stringify(all.filter(t=>t.id!==id)));
    } catch {}
  };
      ...
      {deleteAll && (
        <>
          ...
          <div className="sheet">
            ...
            <button className="btn-p" style={{ background:"rgba(158,96,112,0.15)", borderColor:"rgba(158,96,112,0.3)", color:"var(--wine)" }} onClick={handleDeleteAll}>
              Excluir tudo
            </button>
...
const TelaClinicas = ({ historico, clinicas, setClinicas }) => {
  ...
  const salvar = () => {
    ...
    let next;
    if (editando) {
      next = clinicas.map(c => c.id===editando ? {...c,...form} : c);
    } else {
      next = [...clinicas, { id:`c_${uid().slice(0,8)}`, ...form }];
    }
...
const TelaSemana = ({ historico, clinicas }) => {
  ...
  return (
    <div className="content">
      <div className="ph"><div className="ph-ey">Esta semana</div><div className="ph-title">Dashboard</div><div className="ph-sub">{semT.length} turno{semT.length!==1?"s":""} · {semana.filter(d=>getDT(d).length>0).length} dias trabalhados</div></div>
      ...
      <div className="g2 fu fu1">
        <div className="card cp"><div className="c-ey">Fora de casa</div><div className="c-val-md">{fmtDur(totalFora)}</div><div className="c-sub">esta semana</div></div>
...
const TelaCalculadora = () => {
  ...
  const [vc, setVc]           = useState(300); const [qc, setQc] = useState(10); const [pctC, setPctC] = useState(70);
  const [ve, setVe]           = useState(200); const [qe, setQe] = useState(4);  const [pctE, setPctE] = useState(50);
  const [vl, setVl]           = useState(0);   const [ql, setQl] = useState(0);  const [pctL, setPctL] = useState(60);
  ...
  // Load history from Supabase (or localStorage fallback)
  useEffect(() => {
    loadCalculos()
      .then(data => {
        if (data.length > 0) {
  ...
  const calcular = async () => {
    const pC=pctC/100, pE=pctE/100, pL=pctL/100;
    const liqC=vc*qc*pC, liqE=ve*qe*pE, liqL=vl*ql*pL;
    const bruta=(vc*qc)+(ve*qe)+(vl*ql);
    const liq=liqC+liqE+liqL;
    const final=liq-cost;
    ...
    const result = { local, h, pac, vc, qc, pctC, ve, qe, pctE, vl, ql, pctL, cost, desl,
      bruta, liq, final, hora, horaReal:desl>0?horaReal:null, porPac, classif, classColor,
      liqC, liqE, liqL, ts: Date.now() };
    setRes(result);

    // Save to Supabase + localStorage fallback
    setSavingCalc(true);
    const newHist = [result, ...hist].slice(0, 30);
    setHist(newHist);
    saveCalcHist(newHist); // always keep local copy
    await saveCalculo(result); // Supabase (silently fails if offline)
    setSavingCalc(false);
  };

  const limparHist = async () => {
    setHist([]); saveCalcHist([]); setShowClearHist(false);
  };

  const FldRow = ({ children }) => <div className="frow3" style={{ marginBottom:12 }}>{children}</div>;
  ...
  return (
    <div className="content">
      <div className="ph">
        <div className="ph-ey">Meridian</div>
        <div className="ph-title" style={{ fontStyle:"italic" }}>Calculadora <span style={{ color:"var(--sage)" }}>Valor/Hora</span></div>
      ...
      {res && (
        <div className="calc-res-card mb fu">
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
            <div>
              <div className="c-ey">Valor por hora</div>
              <div className="calc-main-val">{fmtBRL(res.hora)}<span style={{ fontSize:18, letterSpacing:0, fontFamily:"var(--font)", color:"var(--text3)", fontWeight:300 }}>/h</span></div>
              {res.horaReal && <div style={{ fontSize:12, color:"var(--text3)", fontWeight:300 }}>{fmtBRL(res.horaReal)}/h real (c/ deslocamento)</div>}
            </div>
            <div style={{ textAlign:"right" }}>
              <div style={{ fontSize:12, color:res.classColor, fontWeight:500, marginBottom:4 }}>{res.classif}</div>
              <div style={{ fontSize:11, color:"var(--text3)", fontWeight:300 }}>{res.local}</div>
            </div>
          </div>
          <div className="prod-track"><div className="prod-fill" style={{ width:`${Math.min(100,(res.hora/500)*100)}%` }} /></div>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:6, marginTop:12 }}>
            {[
              {l:"Bruto",   v:fmtBRL(res.bruta)},
              {l:"Líquido", v:fmtBRL(res.liq)},
              {l:"Final",   v:fmtBRL(res.final)},
              {l:"Por pac.", v:fmtBRL(res.porPac)},
            ].map(({l,v}) => (
              <div key={l} style={{ background:"var(--bg4)", borderRadius:"var(--r-sm)", padding:"8px 10px", textAlign:"center" }}>
                <div style={{ fontFamily:"var(--font-d)", fontSize:14 }}>{v}</div>
                <div style={{ fontSize:9, color:"var(--text3)", marginTop:2, fontWeight:300 }}>{l}</div>
              </div>
            ))}
          </div>
          {(res.liqC>0||res.liqE>0||res.liqL>0) && (
            <div style={{ marginTop:10, fontSize:11, color:"var(--text3)", fontWeight:300 }}>
              {[res.qc>0&&`Consultas: ${fmtBRL(res.liqC)} (${res.pctC}%)`, res.qe>0&&`Exames: ${fmtBRL(res.liqE)} (${res.pctE}%)`, res.ql>0&&`Laser: ${fmtBRL(res.liqL)} (${res.pctL}%)`].filter(Boolean).join(" · ")}
            </div>
          )}
        </div>
      )}
      ...
      <div className="calc-section mb">
        <div style={{ fontSize:12, color:"var(--text3)", marginBottom:16, fontWeight:300 }}>Insira os dados da sessão para calcular sua rentabilidade real</div>

        <div className="frow" style={{ marginBottom:12 }}>
          <Fld label="Local de trabalho">
            <select className="fi" value={local} onChange={e=>setLocal(e.target.value)}>
              {LOCAIS_CALC.map(l => <option key={l}>{l}</option>)}
            </select>
          </Fld>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
            <Fld label="Horas trabalhadas"><input className="fi" type="number" min="0.5" step="0.5" value={h} onChange={e=>setH(+e.target.value)} /></Fld>
            <Fld label="Nº de pacientes"><input className="fi" type="number" min="0" value={pac} onChange={e=>setPac(+e.target.value)} /></Fld>
          </div>
        </div>

        <div className="calc-group-title">Consultas</div>
        <FldRow>
          <Fld label="Valor (R$)"><input className="fi" type="number" min="0" step="10" value={vc} onChange={e=>setVc(+e.target.value)} /></Fld>
          <Fld label="Quantidade"><input className="fi" type="number" min="0" value={qc} onChange={e=>setQc(+e.target.value)} /></Fld>
          <Fld label="% repasse"><input className="fi" type="number" min="0" max="100" step="5" value={pctC} onChange={e=>setPctC(+e.target.value)} /></Fld>
        </FldRow>

        <div className="calc-group-title">Exames</div>
        <FldRow>
          <Fld label="Valor (R$)"><input className="fi" type="number" min="0" step="10" value={ve} onChange={e=>setVe(+e.target.value)} /></Fld>
          <Fld label="Quantidade"><input className="fi" type="number" min="0" value={qe} onChange={e=>setQe(+e.target.value)} /></Fld>
          <Fld label="% repasse"><input className="fi" type="number" min="0" max="100" step="5" value={pctE} onChange={e=>setPctE(+e.target.value)} /></Fld>
        </FldRow>

        <div className="calc-group-title">Laser / Procedimentos</div>
        <FldRow>
          <Fld label="Valor (R$)"><input className="fi" type="number" min="0" step="50" value={vl} onChange={e=>setVl(+e.target.value)} /></Fld>
          <Fld label="Quantidade"><input className="fi" type="number" min="0" value={ql} onChange={e=>setQl(+e.target.value)} /></Fld>
          <Fld label="% repasse"><input className="fi" type="number" min="0" max="100" step="5" value={pctL} onChange={e=>setPctL(+e.target.value)} /></Fld>
        </FldRow>

        <div className="frow" style={{ marginTop:4 }}>
          <Fld label="Custos associados (R$)"><input className="fi" type="number" min="0" step="10" value={cost} onChange={e=>setCost(+e.target.value)} /></Fld>
          <Fld label="Deslocamento (min)"><input className="fi" type="number" min="0" value={desl} onChange={e=>setDesl(+e.target.value)} /></Fld>
        </div>

        <button className="btn-p" style={{ marginTop:16 }} onClick={calcular}>
      ...
      {hist.length === 0 ? (
        ...
        <div key={i} className="calc-hist-item fu" style={{ animationDelay:`${i*0.03}s` }}>
          <div>
            <div className="calc-hist-val" style={{ color:r.classColor }}>{fmtBRL(r.hora)}/h</div>
            <div className="calc-hist-meta">{r.local} · {r.h}h · {r.qc+r.qe+r.ql} procedimentos</div>
          </div>
          <div style={{ textAlign:"right" }}>
...
export default function App() {
  ...
  const [clinicas,     setClinicas]    = useState(() => loadClinicas());
  ...
  const iniciarTurno = useCallback(async (dados) => {
    const novoTurno = { id:uid(), ...dados, chegadaClinica:null, atendimentoInicio:null, atendimentoFim:null, chegadaCasa:null, status:"ativo" };
    setSaving(true);
  ...
  const encerrarTurno = useCallback(async (extras) => {
    ...
    setTurno(null); setShowEncerrar(false); setTab("historico");
  }, [turno]);
    ...
    <>
      ...
      <div className="app-wrap">
        ...
        <div className="screen">
          {tab==="turno"       && <TelaHome turno={turno} historico={historico} onIniciar={iniciarTurno} onAction={registrarAcao} onEncerrar={()=>setShowEncerrar(true)} saving={saving} clinicas={clinicas} />}
          {tab==="historico"   && <TelaHistorico historico={historico} setHistorico={setHistorico} clinicas={clinicas} />}
          {tab==="analytics"   && <TelaAnalytics historico={historico} clinicas={clinicas} />}
          {tab==="clinicas"    && <TelaClinicas historico={historico} clinicas={clinicas} setClinicas={setClinicas} />}
          {tab==="semana"      && <TelaSemana historico={historico} clinicas={clinicas} />}
          {tab==="calculadora" && <TelaCalculadora />}
        </div>
