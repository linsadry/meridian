...
import { loadTurnos, saveTurno, deleteTurno, saveCalculo, loadCalculos, loadClinicas as loadClinicasDB, saveClinica, deleteClinica, seedClinicas } from "./lib/db";
...
const TelaHistorico = ({ historico, setHistorico, clinicas }) => {
  ...
  const handleDelete = async (id) => {
    // Remove do estado React imediatamente (otimista)
    setHistorico(h => h.filter(t => t.id !== id));
    setConfirmDelete(null);
    // Remove do Supabase + localStorage (db.js cuida dos dois)
  ...
  const handleDeleteAll = async () => {
    setHistorico([]);
    setDeleteAll(false);
    // Limpa localStorage diretamente (mais seguro que Promise.all que pode ter race condition)
    localStorage.removeItem("meridian_turnos_v1");
    // Remove do Supabase em paralelo (silencioso se anônimo)
...
const TelaCalculadora = () => {
  ...
  useEffect(() => {
    loadCalculos()
      .then(data => setHist(data.length > 0 ? data : loadCalcHist()))
  ...
  const calcular = async () => {
    ...
    setHist(newHist);
    saveCalcHist(newHist);
    await saveCalculo(result);
    setSavingCalc(false);
  };

  const limparHist = async () => { setHist([]); saveCalcHist([]); setShowClearHist(false); };
    ...
    <div className="content">
      ...
      <div className="calc-section mb">
        ...
        <div className="frow3" style={{ marginBottom:14 }}>
          ...
          <Fld label="Laser / Procedimentos (R$)">
        ...
        <div style={{ marginBottom:14 }}>
          <Fld label="Deslocamento (min)" hint="ida + volta">
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        setHistorico(concluidos);   // nunca injeta mock — lista vazia é lista vazia
      } catch {
        setHistorico([]);
      }
