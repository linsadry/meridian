...
const TurnoAtivo = ({ turno, onAction, onPausa, onRetomar, onEncerrar, clinicas }) => {
  ...
  return (
    <div className={`shift-card ${getFaseClass(turno)} fu`}>
      ...
      {/* Botão de pausa — só durante atendimento */}
      {emAtendimento && !mostraMotivoInput && (
        <button
          className={`pause-btn${pausaAtiva ? " pausado" : ""}`}
          onClick={pausaAtiva ? onRetomar : handlePausarClick}
        >
          <Icon d={pausaAtiva ? ICONS.play : ICONS.pause} size={14} stroke={1.6}
            color={pausaAtiva ? "var(--sage)" : "var(--amber)"} />
          {pausaAtiva ? "Retomar atendimento" : "Pausar atendimento"}
        </button>
      )}

      {/* Mini-form de motivo da pausa */}
      {mostraMotivoInput && (
        <div style={{
          marginTop:8, background:"rgba(160,140,96,0.06)",
          ...
          <input
            ...
            placeholder="Ex.: sem paciente, intervalo, reunião…"
            value={motivo}
          ...
          <div style={{ display:"flex", gap:8 }}>
            <button className="pause-btn" style={{ flex:1, marginTop:0 }} onClick={confirmarPausa}>
              <Icon d={ICONS.pause} size={13} stroke={1.6} color="var(--amber)" />
              Pausar
            </button>
            <button onClick={cancelarPausa} style={{
              background:"none", border:"1px solid var(--border)", borderRadius:"var(--r)",
              padding:"10px 16px", color:"var(--text3)", fontSize:12, cursor:"pointer",
              fontFamily:"var(--font)"
            }}>
              Cancelar
            </button>
      ...
      {pausas.length > 0 && (
        <div className="pausa-lista">
          {pausas.map((p, i) => (
            <div key={i} style={{ display:"flex", flexDirection:"column", gap:2 }}>
              <div className="pausa-item">
                <span>
                  <Icon d={ICONS.coffee} size={11} stroke={1.4} style={{ marginRight:5, verticalAlign:"middle" }} />
              ...
              {p.motivo && (
                <div style={{ fontSize:10, color:"var(--text3)", fontWeight:300, paddingLeft:14, fontStyle:"italic" }}>
                  "{p.motivo}"
                </div>
              )}
            </div>
          ...
          {totalPausaMs > 0 && (
            <div style={{ fontSize:10, color:"var(--text3)", textAlign:"right", marginTop:2, fontWeight:300 }}>
              Total em pausa: {fmtDur(totalPausaMs)}
            </div>
          )}
        </div>
      )}
...
const TelaHome = ({ turno, historico, onIniciar, onAction, onPausa, onRetomar, onEncerrar, saving, clinicas }) => {
  ...
  return (
    <div className="content">
      ...
      {turno ? (
        <TurnoAtivo turno={turno} onAction={onAction} onPausa={onPausa} onRetomar={onRetomar} onEncerrar={onEncerrar} clinicas={clinicas} />
      ) : (
...
const restoreTurnoAtivo = () => {
  try {
    ...
    // Só restaura se for ativo e de hoje (segurança)
    if (t?.status !== "ativo") return null;
    const saida = new Date(t.saidaCasa);
    const hoje  = new Date();
    if (saida.toDateString() !== hoje.toDateString()) return null;
    return t;
  } catch { return null; }
};
...
export default function App() {
  ...
  useEffect(() => {
    const init = async () => {
      // 1. Restaurar turno ativo salvo localmente (sobrevive a recargas)
      const turnoSalvo = restoreTurnoAtivo();
      if (turnoSalvo) setTurno(turnoSalvo);

      // 2. Carregar clínicas — localStorage primeiro (instantâneo), sem mock
      const clinicasLocais = loadClinicas();
      setClinicas(clinicasLocais);

      // 3. Carregar histórico do Supabase (ou localStorage)
      try {
        const data = await loadTurnos();
        const concluidos = data.filter(t => t.status === "concluido");
        // Se veio um turno ativo do Supabase mais recente, prefere ele
        const ativoRemoto = data.find(t => t.status === "ativo");
        if (ativoRemoto && !turnoSalvo) setTurno(ativoRemoto);
        setHistorico(concluidos.length > 0 ? concluidos : mockHistorico());
      } catch {
  ...
  const iniciarTurno = useCallback(async (dados) => {
    ...
    setTurnoComPersistencia(novoTurno);   // persiste imediatamente
    setSaving(true);
  ...
  const encerrarTurno = useCallback(async (extras) => {
    ...
    setSaving(false);
    persistTurnoAtivo(null);              // limpa o ativo do localStorage
    setTurno(null);
  ...
  // Clínicas: sempre salva localStorage + propaga estado
    ...
    <>
      ...
      <div className="app-wrap">
        ...
        <div className="screen">
          {tab==="turno"       && <TelaHome turno={turno} historico={historico} onIniciar={iniciarTurno} onAction={registrarAcao} onPausa={pausarAtendimento} onRetomar={retomarAtendimento} onEncerrar={()=>setShowEncerrar(true)} saving={saving} clinicas={clinicas} />}
