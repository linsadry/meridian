/**
 * Meridian — Data Layer v2
 * Suporte a missões (viagens multi-dia)
 */
...
const toLocal = (row) => ({
  ...
  // legado: missão inline (turnos antigos)
...
export async function saveTurno(turno) {
  ...
  if (!supabase) return { data: turno, error: null }
...
export async function saveMissao(missao) {
  // LocalStorage imediato — nunca bloqueia
  const all = lsM.load()
  ...
  try {
    ...
    // Nunca deixa o salvamento de missão travar o app
